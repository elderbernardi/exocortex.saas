...
> **⚠️ PATH caveat:** The `mise` shim resolves `browser-use` to Python 3.14 (asyncio incompatible). **Always invoke via the wrapper script or `~/.local/bin/browser-use`**, never the bare command.

### First-time run

```bash
# The wrapper handles everything — just run it:
.agent/skills/browser-use/scripts/browser-use.sh open https://example.com
```
...
```bash
BU=".agent/skills/browser-use/scripts/browser-use.sh"
