---
capitulo: 3
titulo: "O Exocórtex.IA — O Framework"
natureza: framework
dependencias: []
status: rascunho
---
...
```mermaid
flowchart TD
  subgraph Macroverso ["MACROVERSO<br>(Quem você é. Suas leis. Sua identidade.)"]
    subgraph Microverso ["MICROVERSO<br>(O cenário atual. O domínio. O par.)"]
      Tarefa["TAREFA<br>(O que precisa ser feito.)"]
    end
  end
```
...
```mermaid
flowchart BT
  IA["IA"] -- "questiona, desafia, explica, refina\n(Vetor de Evolução)" --> HUMANO["HUMANO"]
  HUMANO -. "Resultado: conhecimento, clareza, habilidade" .-> HUMANO
```
...
```mermaid
flowchart TB
  HUMANO["HUMANO"] -- "intenção, direção, validação\n(Vetor de Execução)" --> IA["IA"]
  IA -- "processa, gera, estrutura, entrega" --> Artefato["Artefato (código, texto, plano, design)"]
```
...
<div class="canvas-cognitivo">
  <div class="canvas-title">CANVAS COGNITIVO</div>
  <table>
    ...
    <tr><th>NATUREZA:</th><td>[ ] Aprender &nbsp; [ ] Aprofundar &nbsp; [ ] Criar<br>[ ] Decidir &nbsp; [ ] Revisar</td></tr>
...
```mermaid
flowchart TD
  EU["EU"] --> MACRO["MACROVERSO<br>🏷️ 📚 📝 🎭 🪞"]
  EU --> MICRO_A["MICROVERSO: A<br>🏷️ 📚 📝 🎭 ⚙️ 🔧"]
  EU --> MICRO_B["MICROVERSO: B<br>🏷️ 📚 📝"]
  
  TAREFA["🎯 TAREFA ATUAL<br><i>(usa e referencia artefatos de cima)</i>"]
...
```mermaid
flowchart TD
  MARINA["MARINA"] --> MACRO["MACROVERSO"]
  MARINA --> MICRO_1["MICRO: Hist. 8º Ano"]
  MARINA --> MICRO_2["MICRO: Pesquisa Ed."]
  
  MACRO --> C1["🏷️ Prof Hist 12a exp"]
  MACRO --> C2["📚 Não aceito lúdico genérico"]
  MACRO --> C3["📝 Peço para repetir antes"]
  MACRO --> C4["🪞 Qdo declaro exp, IA muda tom"]
  
  MICRO_1 --> M1_1["🏷️ 32 alunos, 2 TEA esc. públ"]
  MICRO_1 --> M1_2["📚 Avaliação formativa funciona qdo..."]
  MICRO_1 --> M1_3["📝 Prompt avaliação inclusiva"]
  
  MICRO_2 --> M2_1["🎭 Supervisor Pedagógico"]
  MICRO_2 --> M2_2["⚙️ Workflow: criar avaliação"]
  MICRO_2 --> M2_3["🔧 Deep Research p/ base teórica"]
```
...
```mermaid
flowchart TD
  DANIEL["DANIEL"] --> MACRO["MACROVERSO"]
  DANIEL --> MICRO_1["MICRO: Proposta Roteirização"]
  DANIEL --> MICRO_2["MICRO: Governança Corporativa"]
  
  MACRO --> C1["🏷️ Ger. Oper. Log. direta"]
  MACRO --> C2["📚 Não embelezo situação"]
  MACRO --> C3["📝 Uso dados reais"]
  MACRO --> C4["🪞 Dados sem fonte são ruído"]
  
  MICRO_1 --> M1_1["🏷️ Frota 120 veíc. 3 CD"]
  MICRO_1 --> M1_2["📚 Custo erro R$340k/mês"]
  MICRO_1 --> M1_3["📝 Template apres. c/ ROI"]
  
  MICRO_2 --> M2_1["🎭 CFO Conservadora"]
  MICRO_2 --> M2_2["🎭 CEO Visionário"]
  MICRO_2 --> M2_3["📚 Histórico de decisões da dir."]
```
...
```mermaid
flowchart TD
    A["🧠 1. PENSAR\nO que eu preciso?\nO que eu já sei?\nO que me falta?"] --> B["💬 2. CONVERSAR\nExplicar o cenário para a IA.\nPerguntar, ouvir, corrigir.\nEla entendeu?"]
    B -->|Não entendeu| B
    B -->|Entendeu| C["⚡ 3. FAZER\nPedir a geração ou\ncontinuar aprendendo.\nAvaliar: ficou bom?\nTem a minha cara?"]
    C -->|Não ficou bom| B
    C -->|Ficou bom| D["📝 4. GUARDAR\nO que aprendi?\nO que deu certo?\nO que uso de novo?"]
```
...
```mermaid
flowchart TD
    A["1. MAPEAR\nOnde estou? O que preciso?"] --> B["2. CARREGAR\nAlimentar o contexto"]
    B --> C{"3. VALIDAR\nA IA entendeu?\nEu entendo o suficiente?"}
    C -->|"Não (refinar)"| B
    C -->|Sim| D{"4. ESCOLHER VETOR\nPreciso aprender\nou produzir?"}
    D -->|Evolução| E["5a. EVOLUIR\nDiálogo socrático,\ndesafio, expansão"]
    D -->|Execução| F["5b. EXECUTAR\nGeração do artefato"]
    E --> G{"6. AVALIAR\nAprendi o suficiente?\nReconheço minha autoria?"}
    F --> H{"6. AVALIAR\nA qualidade supera\no que eu faria sozinho?\nReconheço minha autoria?"}
    G -->|"Preciso de mais contexto"| B
    G -->|"Preciso mudar de vetor"| D
    G -->|"Não, iterar"| E
    G -->|Sim| I["7. REGISTRAR\nCapturar para reuso"]
    H -->|"Preciso de mais contexto"| B
    H -->|"Preciso mudar de vetor"| D
    H -->|"Não, iterar"| F
    H -->|Sim| I
    I --> J["8. EVOLUIR O SISTEMA\nO que aprendi sobre\ncomo uso o framework?\nO que posso guardar para reuso?"]
```
