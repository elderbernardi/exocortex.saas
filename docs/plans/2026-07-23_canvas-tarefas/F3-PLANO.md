...
| `$HERMES_HOME/SOUL.md` (runtime, isolated for smoke) | **propagate** | Receives `SOUL_SEED.md` **verbatim** via `setup.sh` step-07 (or `cp SOUL_SEED.md $HERMES_HOME/SOUL.md`). Mandatory 2nd hop — `compile_soul → SOUL_SEED.md` alone does NOT reach the launched session (SOUL trace). — NB (C0): esse `cp`/step-07 apaga o onboarding num install vivo; a propagação correta é `compile_soul.py --soul "$HERMES_HOME/SOUL.md"` (cirúrgico). O step-07/cp só vale no smoke isolado (é o que T15/T16 usam). |
