...
ALLOWED_SYNC_PREFIXES=(
  "skills/exocortex/excrtx-"     # Skills
...
instance_to_repo_path() {
  ...
  # Skills: $HERMES_HOME/skills/exocortex/<skill>/ → skills/<skill>/
  if [[ "$ipath" == *"/skills/exocortex/"* ]]; then
    echo "skills/${ipath##*/skills/exocortex/}"
    return
  fi
...
if [ -n "$REPO_PATH" ] && [ -d "$REPO_PATH/.git" ]; then
  ...
  for manifest in "${pending_manifests[@]}"; do
    ...
    instance_skill_dir="$HERMES_HOME/skills/exocortex/$skill_name"
    ...
    if [ -d "$instance_skill_dir" ] && [ -d "$repo_skill_dir" ]; then
      ...
      while IFS= read -r file; do
        ...
        # Check allowlist
        if ! is_path_allowed "skills/exocortex/$skill_name/$rel_path"; then
