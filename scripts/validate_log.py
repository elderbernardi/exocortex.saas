...
def validate_log(path):
    ...
    # Helper to get first non-empty line from a list of lines
    def first_nonempty(lns):
        for ln in lns:
            if ln.strip():
                return ln.strip()
        return ""
    ...
    # ---- §1.1 - H1 check ----
    first_line = first_nonempty(body_lines)
    if not first_line:
        # empty body after frontmatter
        pass  # will be caught by missing H1? We'll let later checks handle.
    if not H1_RE.match(first_line):
        issues.append(("L-001", "ERROR", "file should start with an H1 '# Log' line (§1.1)"))
    if first_line and not H1_CANONICAL_RE.match(first_line):
        issues.append(("L-003", "ERROR", "H1 should be '# Log' or '# Log — <name>' (§1.1)"))

    # Check for multiple H1s
    h1_indices = [i for i, ln in enumerate(body_lines) if H1_RE.match(ln)]
    if len(h1_indices) > 1:
        issues.append(("L-002", "ERROR", f"exactly one H1 expected, found {len(h1_indices)} (§1.1)"))

    # ---- Scan body for headings, bullets, etc. ----
    seen_dates = []
    has_legacy_heading = False
    current_date = None
    in_fence = False
    first_h1_seen = False
    # We'll process body_lines index = 0
    for idx, ln in enumerate(body_lines):
        ...
        if h1:
            first_h1_seen = True
            # already checked first line; continue
            continue
        ...
        if h2:
            ...
            if not m:
                ...
                issues.append(("L-010", "ERROR", f"date heading not '## YYYY-MM-DD' (legacy/free-text): {ln.strip()!r}"))
                current_date = None
                continue
            d = m.group(1)
            if not _is_valid_date(d):
                issues.append(("L-012", "ERROR", f"invalid calendar date in heading: {d}"))
            if seen_dates and d < seen_dates[-1]:
                issues.append(("L-011", "ERROR", f"date headings must be ascending; {d} follows {seen_dates[-1]} (§1.2)"))
            seen_dates.append(d)
            current_date = d
            continue
        ...
        if bullet:
            ...
            # Only check entry format if we have seen an H1 and no legacy heading yet (to avoid noise on legacy files)
            if first_h1_seen and not has_legacy_heading:
        ...
        if SUBBULLET_RE.match(ln):
            issues.append(("L-020", "ERROR", f"sub-bullets / continuation lines are not allowed (§1.1): {ln.strip()!r}"))
            continue

        # Anything else under the body is free text.
        if first_h1_seen:
            issues.append(("L-020", "ERROR", f"free text not allowed between/under headings (§1.1): {ln.strip()!r}"))

    # ---- §3.3 - macro/ must not have a log ----
    parts = Path(path).parts
    if "macro" in parts:
        issues.append(("L-030", "ERROR", "macro/ must NOT have a log.md (§3.3 — identity tracked in git only)"))
...
def main(argv=None):
    ...
    parser.add_argument("--dir", metavar="PATH", help="Find and validate all log.md files in a tree.")
