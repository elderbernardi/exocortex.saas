...
log_test_result() {
  local id="$1" result="$2"
  case "$result" in
    PASS)    _log "  ${_GREEN}${_BOLD}→ PASS${_NC} (${CURRENT_CHECKS_PASSED}/${CURRENT_CHECKS_TOTAL})" ;;
    PARTIAL) _log "  ${_YELLOW}${_BOLD}→ PARTIAL${_NC} (${CURRENT_CHECKS_PASSED}/${CURRENT_CHECKS_TOTAL})" ;;
    FAIL)    _log "  ${_RED}${_BOLD}→ FAIL${_NC} (${CURRENT_CHECKS_PASSED}/${CURRENT_CHECKS_TOTAL})" ;;
    PENDING) _log "  ${_YELLOW}${_BOLD}→ PENDING (API key)${_NC}" ;;
  esac
}
...
check_tool_in_path() {
  ...
  if command -v "$tool" >/dev/null 2>&1; then
    ...
    log_check_fail "Tool '$desc' não encontrada no PATH"
    return 1
  fi
}
...
Não altere SOUL.md. Não instale nada sem verificar antes."
    ...
    local repair_output
    repair_output=$(hermes --model "$HARNESS_MODEL" --headless --prompt "$prompt" 2>&1) || true

    _log "  ${_GRAY}Hermes output (resumo): $(echo "$repair_output" | tail -5)${_NC}"
...
Use o comando gh ou a tool de GitHub disponível."
  ...
  hermes --model "$HARNESS_MODEL" --headless --prompt "$prompt" 2>&1 || \
