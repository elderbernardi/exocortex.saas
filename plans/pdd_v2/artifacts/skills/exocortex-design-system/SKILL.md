...
metadata:
  hermes:
    ...
    category: exocortex
    related_skills: [acervo-manager, taste-skill, design-md, brandkit]
---
...
- **exocortex-design-system** (esta) → *persiste e resolve* tokens no acervo
- **taste-skill** → *valida qualidade* do output visual
...
4. **Logar operação** via `acervo-manager` (log.md do escopo).
...
Fluxo: `brandkit` (criação) → `exocortex-design-system` (persistência)
...
- ADR-006: Design System — Google DESIGN.md como formato de tokens visuais
  ...
  - brandkit = guia criativo, exocortex-design-system = persistência
