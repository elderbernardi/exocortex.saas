...
6. Para publicação de artefato final, encaminhar para `personal-artifact-workspace` e usar `artifact_publish.py` (manifest + receipt + resolução de pasta). Não publicar final com `google_api.py drive upload` direto.
