...
nlm login --check
```
3. Verificar binding Hermes quando houver uso MCP:
```bash
hermes mcp list
```
Confirmar presença de `notebooklm` habilitado.
4. Se auth falhar: executar `nlm login` e conduzir fluxo remoto via chat (URL de autorização + URL final colada).
5. Se MCP não estiver habilitado, registrar fallback explícito para trilha CLI-only antes de continuar.

Regra operacional: primeiro garantir cadeia de aquisição (fonte + acesso + integração), depois iniciar síntese e learning cards.
