...
```text
input → Canvas → Tarefa → Execução/Evolução/Manutenção → Artefato/Decisão/Atividade → Avaliação → Persistência → Publicação opcional
```
...
| Atividade | Movimento de manutenção/zeladoria | `acervo/_activities/{activity_id}/` quando recorrente/auditável | //TODO atividade pode confundir com "faça atividade sobre assunto..." . Pensar outro substantivo. Pensei em rotina 
| Automação | Gatilho de atividade/persona | `acervo/_automations/{automation_id}.yaml` | //TODO rotina
...
Pensar, formular, tensionar, explorar possibilidades, explicitar premissas, gerar decisões candidatas.
