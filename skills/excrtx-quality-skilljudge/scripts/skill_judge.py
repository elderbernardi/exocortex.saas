...
def main():
    ...
    for skill_path in skill_paths:
        ...
        if args.d1_only:
            ...
            # Build prompt for LLM evaluation (to be called externally)
            prompt = build_judge_prompt(parsed, rubric_text, soul_context)
            # In d1-only mode we skip LLM. In full mode, the prompt is printed
            # for external LLM invocation or piped to an LLM API.
            llm_dims = {}  # placeholder — filled by external LLM call
